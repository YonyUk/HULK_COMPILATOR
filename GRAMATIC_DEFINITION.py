import HULK_LANGUAGE_DEFINITION as HK

gramar =[


# function_caLL
[
    ["F",[ ["c","P"] , ["c","N" ] , ["c","T"] ] ],
    ["P",[["(","p",")"]]],
    ["T",[["F"]],],
    ["N",[["(",")"]]],
],

# strings
[
    ["T", [ ["T","@","T"] ,["F","@","T"], ["T","@@","T"] , ["F","@@","T"]] ],
    ["E", [["T","@","E"] ,["F","@","E"],["T","@@","E"] , ["F","@@","E"] ] ]
],

# expression_block
[
    ["O",[["E","E"], ["T","$2","O"] ,["O","E"],["O","B"],["O","$2","b"],["b","$2","E"]]],
    ["O",[["O","$2","E"],["E","$2","b"] , ["E","$2","O"],["O","$2"] , ["O",";"],["E","$2","E"],["b","$2","b"]]],
    ["b",[["{","O","}"],["{","E","}"],["{","B","}"],["{","}"],["b","$2"],["{","b","}"],["{","T","}"]]],
    ["B",[["b",";"]]],
    ["T",[["T","$2"]]],
    ["E",[["E","$2"],["B"]]],
    ["E",[["$2",";","$3"],["E",";"]]],
    
    ["b",[["{","M","}"]]],
    
    ["E" , [["T",";"]]],
    ["T",  [["T" ,".","E"] , ["F" ,".","E"], ["T" ,".","T"] , ["F" ,".","T"] ]],
    ["E",  [["T" ,".","E"] ]]
],

# literals
[

["T" , [ ["let","T"] ]],
["p" , [ ["T", "," ,"T" ] , ["T",",","p"] ]],
["T" , [ ["T",":=","T"] ]],
["E" , [ ["T",":=","E"] ]],
["T" , [ ["T","=","T"]  ]],
["E" , [  ["T","=","E"] ]],
["T" , [ ["T","as","T"] , ["F","as","T"] , ["T","as","E"] ]],
["E" , [ ["T","as","E"] ]],
["E" , [ ["T",":","E"] ]],
["E", [["T",";","$2"]]],
["T", [["(","T",")"]]],

],

# booleans
[
    
    ["T" , [["T","&","T"] , ["F","&","T"] , ["T","|","T"] , ["F","|","T"]  , ["T","!=","T"] , ["F","!=","T"] , 
            ["T",">","T"] , ["F",">","T"]  , ["T","<","T"] , ["F","<","T"]  , ["T","<=","T"] , ["F","<=","T"] ,
            ["T",">=","T"] ,["F",">=","T"] , ["T","==","T"] ,["F","==","T"] , 
            ["T","is","T"] , ["F","is","T"] ]],
    
    ["E" , [["T","&","E"]  , ["T","|","E"]  , ["T","!=","E"] ,
            ["T",">","E"]  , ["T","<","E"]  , ["T","<=","E"] ,
            ["T",">=","E"] , ["T","==","E"] , 
            ["T","is","E"] ]],
],

# numbers
[
    
    ["T" , [["T","+","T"] , ["T","-","T"],[ "T","*","T"], ["T","/","T"], ["T","^","T"], ["T","%","T"]]],
    ["T" , [["F","+","T"] , ["F","-","T"],[ "F","*","T"], ["F","/","T"], ["F","^","T"], ["F","%","T"]]],
    ["E" , [["T","+","E"], ["F"+"E"],["T","-","E"],[ "T","*","E"], ["T","/","E"], ["T","^","E"], ["T","%","E"]]],
    
    ["T" , [ ["T","-=","T"] ,["T","+=","T"] ,["T","/=","T"] ,["T","*=","T"] , ["T","--"] , ["F","--"] , ["T","++"] ,["F","++"] ]],
    ["E" , [ ["T","-=","E"] ,["T","+=","E"] ,["T","/=","E"] ,["T","*=","E"] , ["E","--"] , ["E","++"] ]]
],

# IN
[
    ["p",[["p","$2"]]],
    ["T", [["T","in","T"] , ["p","in","T"] , ["p","in","p"]]],
    ["T", [["F","in","T"],["T","$2"]]],
    ["E", [["T","in","E"] ,["T","in","b"] , ["p","in","E"]]],
       
],

#  For
[
    ["E" , [["for","T","$2","B"] , ["for","T","$2","E"]]],
    ["E", [ ["for","T","$2","b"]]],
    
],

# conditional
[
    
    ["if",[["if","T","$2","E"],["if","T","$2","b"],["if","T","$2","B"],["if","$2"]]],
    
    ["elif",[["if","elif","T","E"],["if","elif","T","b"],["if","elif","T","B"],["elif","$2"]]],
    
    ["E",[["if","else","E"],["if","else","b"],["if","else","B"]]],
    
    ["E",[["elif","else","E"],["elif","else","b"],["elif","else","B"]]],
    
    ["E",[["if","else","E"],["if","else","b"],["if","else","B"]]],
    
],

# While
[
    
    ["E" , [["while","T","$2","B"] , ["for","T","$2","E"]]],
    ["E", [ ["while","T","$2","b"]]],
],

# function
[    
 
    ["M" , [ ["function","T","$2","=>","E"] , ["function","T","$2",":","T" ,"=>","E"] 
            ,["function","T","$2","=>","b"],["function","T","$2",":","T","=>","b"] ,
            [ 'function', 'T',"$2", ':', 'T', 'b'],['function', 'T', '$2', 'b', ]]],
    
    ["M",[["M","$2"],["M","$2","M"]]]
],

# types
[
    ["M ", [["type","T","$2","b"  ], ["type","T","$2","inherits","T","$2","b"] ]],
    ["T" , [ ["new","F"] ]],
],

# protocols
[
    ["M" , [["protocol","T","$2","b"] ,[ "protocol","T","$2","extends","T","$2","b"]]]
],

# vector
[
    
    ["T" , [[ "[","T","||","T" , "]"] , [ "[" , "p" , "]" ] , ["[" , "T" , "]" ]  ]],
]
]

def traslator(token_list):
    
    parse_list=["$1"]
    parse_list.append("$2")
    
    index=0
    while index < len(token_list ):
        
        if token_list[index] =="'" or token_list[index] == "\"" : 
            
            index1 = index + 1
            while index1 < len(token_list):
                
                if token_list[index1] == "'" or token_list[index1] == "\"" :
                    parse_list.append("T")
                    index = index1
                    index += 1
                    break
                
                index1 +=1
                    
        kw = False                    
        for arg in HK.SYMBOLS_and_OPERATORS_parser:            
            
            if token_list[index] == arg:
            
                if arg == "in":
                    parse_list.append("$2")
                    
                parse_list.append(token_list[index])
                
                if arg == ";" or arg == ")" or arg == "}" or arg == "]":
                    parse_list.append("$2")
                
                kw =True
                break
            
        if not kw:
            if index + 1 < len(token_list) and token_list[index + 1 ] == "(" :
                parse_list.append("c")
            
            else:
                parse_list.append("T")
        index += 1
    
    parse_list.append("$3")
    
    return parse_list
